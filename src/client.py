...
from treecoviz import CommonTree
import matplotlib
...
cmp_btn, set_btn, auto_fig_btn, errors = None, None, None, None
sliders = []
answer = None
...
def compare(newick, plot_options, fig_options, plot_colors):
	for _, slider in enumerate(sliders):
		if slider.validation(fig_options[_]) is not None:
			return -1, -1, -1
	try:
		tmp = CommonTree().main(*newick, *plot_options, *fig_options, plot_colors)
		return tmp
...
async def execute_compare():
	...
	res, n, hop = await run.cpu_bound(compare, newick, plot_options, fig_options, plot_colors)
	if res == -1:
...
with ui.row():
	...
	with inputs_card:
		...
		with ui.row():
			...
			set_btn = ui.button("Settings", icon="settings", on_click=lambda: settings_pt.set_visibility(True))
			# ui.space()
		...
		with settings_pt:
			...
			with ui.row(align_items="center").bind_visibility_from(show_plotoptions, "value"):
				with ui.row(align_items="center"):
					...
					ui.space()
					auto_fig_btn = ui.checkbox("Custom Figure Size", value=False, on_change=lambda x: fig_options.__setitem__(3, x.value))
				with ui.row(align_items="center").bind_visibility_from(auto_fig_btn, "value"):
