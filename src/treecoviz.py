...
class CommonTree():
	...
	class tree:
		...
		def print_tree(self, root):
			def dfs(cur: CommonTree.node, dep: int):
				print(f"{'  '*dep}{cur.name} ({cur.label})")
		...
		def write_vector(self, root):
			def dfs(cur: CommonTree.node, key: int):
				if key in cur.label:
					return []
				return dfs(cur.fa, key) + [cur.label]
			res = []
			for i in range(1, len(self.leaves)+1):
				res.append(dfs(self.leaves[self.leaves_ref[i]].fa, i))
			return res
	...
	def tree_align(self, t1: tree, t2: tree):
		for i in range(1, len(t1.leaves)+1):
			x = t1.nonleaves[t1.nonleaves_ref[i]]
			z = t2.nonleaves[t2.nonleaves_ref[i]]
			if x.fa == None:
				# print(f"NOT split from {x.name} (label {x.label}): x.fa == None")
				continue
			t = (set(x.label) | set(x.fa.label)) & set(z.label)
			if t.issubset(set(x.label)) or t.issubset(set(x.fa.label)):
				# print(f"NOT split from {x.name} (label {x.label}): {t}")
				continue
			t = (set(x.label) & set(z.label)) | set([x.u])
			if len(t) == 1:
				# print(f"NOT split from {x.name} (label {x.label}): {t}")
				continue
			
			# print(f"split from {x.name} (label {x.label}): {t}")
			t2.inc += 1
			y = self.node()
			y.name = f"i{t2.inc}"
			y.length = 0
			t2.nonleaves[y.name] = y
			for ch in z.children:
				if ch.u in t:
					y.children.append(ch)
					ch.fa = y
			for ch in y.children:
				z.children.remove(ch)
			z.children.append(y)
			y.fa = z
			y.label = sorted(list(t))
			y.u = y.label[0]
			y.label = y.label[1:]
			for ch in y.label:
				z.label.remove(ch)
				t2.nonleaves_ref[ch] = y.name
		for i in range(1, len(t2.leaves)+1):
			x = t2.nonleaves[t2.nonleaves_ref[i]]
			z = t1.nonleaves[t1.nonleaves_ref[i]]
			if x.fa == None:
				continue
			t = (set(x.label) | set(x.fa.label)) & set(z.label)
			if t.issubset(set(x.label)) or t.issubset(set(x.fa.label)):
	...
	def run(self, s1: str, s2: str):
		...
		self.t1.newick_to_vector(s1)
		self.t2.newick_to_vector(s2)
		# self.t1.print_tree(self.t1.root)
		# self.t2.print_tree(self.t2.root)
		self.tree_align(self.t1, self.t2)
		# self.t1.print_tree(self.t1.root)
		# self.t2.print_tree(self.t2.root)
		self.a = self.t1.write_vector(self.t1.root)
		self.b = self.t2.write_vector(self.t2.root)
		# print(self.a)
		# print(self.b)
	...
	# Outputs
	def plot_solution(self, write_length: bool, format: str, plt_num: int, fig_height: int, fig_width: int, fig_dpi: int, custom_fig: bool, plt_colors: list) -> None:
		...
		label_size = 20 if n <= 10 else \
					 ...
					 7	if n <= 500 else 6
		if custom_fig == False:
			fig_height = fig_width = 10 if n <= 80 else \
									 15 if n <= 120 else \
									 20 if n <= 200 else 30
			fig_dpi = 60 if n <= 80 else 120
	...
	def main(self, s1: str, s2: str,
		...
		T_start: float = 1.
		T_end: float = 1e-3
		cooling_rate: float = max(0.9, math.tanh(math.log(len(ans_fa)) / 1.8))
		max_iter: int = max(300, int(len(ans_fa) * 2.5))
		...
		self.ans_t1 = self.tree(self)
		self.ans_t2 = self.tree(self)
		self.ans_t1.newick_to_vector(s1)
		self.ans_t2.newick_to_vector(s2)
		# print(self.ans_a)
		# print(self.ans_b)
		# print(self.ans_ord)
		# print(self.ans_fa)
		# self.ans_t1.print_tree(self.ans_t1.root)
		# self.ans_t2.print_tree(self.ans_t2.root)
		# plot
		self.plot_solution(write_length, format, plt_num, fig_height, fig_width, fig_dpi, custom_fig, plt_colors)
...
if __name__ == "__main__":
	...
	# s2 = "(a,b,c,d,e)"
	cmt = CommonTree()
	cmt.main(s1=s1, s2=s2, format="png")